...
const METRONOME_PRESETS = [0.5, 0.75, 1, 1.25, 1.5, 2]
...
export default function SettingsPanel(props: SidebarProps) {
  ...
  const metronomePresetItems = [
    ...METRONOME_PRESETS.map((value) => ({
      id: value.toString(),
      name: value === 0.5 ? '1/2×' : value === 0.75 ? '3/4×' : `${value}×`,
    })),
  ...
  const handleRestoreTrack = (trackId: number) => {
    if (!props.song) {
      return
    }
    const defaultTrack = getDefaultSongSettings(props.song).tracks[trackId]
    if (!defaultTrack) {
      return
    }
    updateTrack(trackId, defaultTrack)
  }
      ...
      <div className="flex-1 overflow-y-auto pt-0">
        <Section title="Playback" icon={<AudioWaveform className="h-4 w-4 text-violet-300" />}>
          ...
          <SettingRow
            ...
            title="Key signature"
            subtitle="Affects sharps/flats display"
          >
            <Select
        ...
        <Section title="Tracks" icon={<ListMusic className="h-4 w-4 text-violet-300" />}>
          <div>
              ...
              {trackEntries.map((entry, index) => {
                ...
                return (
                  <div
                    key={entry.id}
                    className={clsx(
                      'group rounded-md border border-transparent bg-[#1f1e26] p-3 transition-all hover:border-[#3a3444]',
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          className="ml-1 flex items-center justify-center text-gray-500 opacity-0 transition-all group-hover:opacity-100 hover:text-[#8b5cf6]"
                          title="Sample Sound"
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex rounded-md bg-black/40 p-0.5">
                          <button
                            ...
                            className={clsx(
                              ...
                              entry.track.hand === 'left' && 'bg-blue-600 text-white',
                            )}
                          ...
                          <button
                            ...
                            className={clsx(
                              'rounded px-2 py-1 text-[10px] font-semibold text-gray-500 transition-all',
                              entry.track.hand === 'right' && 'bg-red-500 text-white',
                            )}
                        ...
                        <TooltipTrigger delay={0}>
                          <button
                            type="button"
                            className="flex items-center justify-center text-gray-500 opacity-0 transition-all group-hover:opacity-100 hover:text-[#8b5cf6]"
                            onClick={() => handleRestoreTrack(entry.id)}
                          >
                            <Undo2 className="h-4 w-4" />
                          </button>
                          <Tooltip>Reset track</Tooltip>
                        </TooltipTrigger>
                      </div>
                    </div>
                    <Select
                      aria-label={`Instrument for track ${index + 1}`}
                      className="w-full"
                      size="md"
                      selectedKey={entry.track.instrument}
                      onSelectionChange={(key) =>
                        handleSelectInstrument(entry.id, key as InstrumentName)
                      }
...
function Section({ children, title, icon, defaultOpen = false }: SectionProps) {
  return (
    <details
...
function formatInstrumentName(name: string) {
  return name
    .replace(/_/g, ' ')
    .split(' ')
