  ...
  return (
    ...
    >
      ...
      <Button className={(renderProps) => styles({ ...renderProps, size })}>
        <SelectValue className="flex-1 min-w-0 truncate text-zinc-200" />
        {isLoading ? (
          <LoaderCircle className="absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2 animate-spin text-zinc-400" />
        ) : (
          <ChevronDown
            aria-hidden
            className="absolute right-2 top-1/2 h-2.5 w-2.5 -translate-y-1/2 text-zinc-400 group-disabled:text-zinc-500"
          />
        )}
      </Button>
